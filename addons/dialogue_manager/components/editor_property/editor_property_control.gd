...
func _on_file_dialog_new_file_selected(file: String) -> void:
	...
	if editor_plugin.dialogue_file_cache.has(file):
		emit_signal("resource_changed", load(file))
	else:
		var next_resource: Resource = await editor_plugin.import_plugin.compiled_resource
		next_resource.resource_path = file
		emit_signal("resource_changed", next_resource)
...
func _on_files_list_file_double_clicked(file_path: String) -> void:
	emit_signal("resource_changed", load(file_path))
...
func _on_quick_open_dialog_confirmed() -> void:
	if quick_selected_file != "":
		emit_signal("resource_changed", load(quick_selected_file))
